...
class Pi_Corw_Analytics{
    ...
    public function show_tracker_notice() {
        ...
        $notice .= '<p>'.esc_html__("Hi, your support can make a big difference!", 'cancel-order-request-woocommerce').'</p>';
        $notice .= '<p>'.esc_html__("We collect only technical data — including the plugin version, WordPress version, WooCommerce version, and site url — solely to improve compatibility and enhance plugin features.", 'cancel-order-request-woocommerce').'</p>';
        ...
        $notice .= sprintf(
            ...
            esc_html__('I Don\'t Help', 'cancel-order-request-woocommerce')
        );
        $notice .= sprintf(
            ...
            esc_html__('I Will Help', 'cancel-order-request-woocommerce')
        );
    ...
    public function handle_deactivation_form() {
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'pi_deactivate_nonce_' . $this->plugin_slug)) {
            wp_die(esc_html__('Security check failed', 'cancel-order-request-woocommerce'));
        }
        ...
        if (is_multisite() && is_plugin_active_for_network($this->plugin_path)) {
            if (is_super_admin()) {
                ...
                wp_die(esc_html__('You do not have permission to deactivate a network plugin.', 'cancel-order-request-woocommerce'));
            }
        } else {
